    ...
    <>
      ...
      */}
      ...
      <DraggablePanel
        ...
        /*
          Ключ сховища піднято до v2 НАВМИСНО. У кожного, хто вже відкривав
          вікно набору, у localStorage лежить збережений прямокутник 1280×860,
          і без нового ключа зміна нижче не подіяла б узагалі: збережений
          розмір перемагає початковий. Старий запис лишається недоторканим,
          просто більше не читається.
        */
        storageKey="nova.editor.window.v2"
        /*
          На всю ширину екрана, а не 1280 px. Вікно набору — режим «нічого
          зайвого», але всередині нього поруч із текстом стоять зміст книги
          ліворуч і панель персонажів праворуч: фіксована ширина з'їдалась
          ними, і сам текст лишався вузькою смугою посередині, ще й з
          порожніми полями обабіч вікна на широкому моніторі.
          `DraggablePanel` сам затискає розмір до `innerWidth/innerHeight − 24`
          і центрує вікно, тож тут передається саме розмір вікна браузера, а
          не число зі стелі. Це ПОЧАТКОВИЙ розмір: перетягнути й зменшити
          вікно письменник може як і раніше, і його вибір збережеться.
        */
