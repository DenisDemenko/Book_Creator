...
const CORE_MODULE_LABELS: Record<CoreModuleKey, string> = {
  ...
  bookPdfDesign: 'Дизайн макета PDF книги',
  etsyAdvisor: 'Консультант з Etsy-аналітики',
  emotionMastery: 'Емоційна майстерність письменника',
  threshold: 'Поріг',
  instructionElaboration: 'Поглиблення кроків інструкції (Інструкція)',
...
const CORE_MODULE_DESCRIPTIONS: Record<CoreModuleKey, string> = {
  ...
  coreAi2Analysis:
    'Роль AI-2 семантичного ядра: спеціалізований аналіз за завданням (персонаж, емоції, безперервність, стиль, переклад, тлумачення запиту). Та сама схема й ті самі правила доказу, що в AI-1.',
  coreAi3Visual:
    'Роль AI-3 семантичного ядра: ілюстрації книги — ознаки зовнішності, предмети, локації, порівняння з описом у тексті. Потрібна модель із зором. Доказ — id зображень чи абзаців із вхідних даних.',
  ...
  diagnStyle:
    'Підмодуль /diagn: авторський стиль — синтаксис, лексика, ритм, діалоги. Повертає метрики 0-100 і цитати з тексту; схема не редагується, бо за нею малюються прогрес-бари картки.',
  diagnStructure:
  ...
  behaviorDrift:
    'Вужчий за «Хранителя цілісності»: лише заявлені патерни поведінки персонажа + уривки тексту → окрема оцінка КОЖНОГО патерну (підтверджується / дрейфує / замало даних). Схема відповіді не редагується.',
  readerResponse:
  ...
  etsyAdvisor:
    'Задача (ціна / тренд / лістинг / магазин) плюс дані по ній → коротка порада практика. Один модуль на чотири задачі Etsy-аналітики: різниця між ними у питанні, а не в ролі моделі. Схема відповіді вільна — це текст, а не структура.',
  bookPdfDesign:
    'Опис книги (жанр, обсяг, зразок тексту) → макет друкованого PDF: формат, поля, кегль, інтерліньяж, абзацний відступ, розміри заголовків. Модель НЕ робить сам файл — вона віддає лише специфікацію макета, а рендерить її код Студії (той самий, що й у варіанті «макет від коду»). Усі числа затискаються в читабельні межі: кегль поза 8–16 пт чи поля поза 20–100 пт будуть відкинуті. Схема відповіді не редагується.',
  etsyMarketScreen:
  ...
  emotionMastery:
    'Виділений фрагмент тексту → основна/вторинні/приховані емоції персонажа й 10 критеріїв майстерності передачі (0-10 кожен, з доказом-цитатою). Підсумковий mastery_score рахує сервер, а не модель. Дописує доказ у навичку WDI «Емоційний вплив». Схема відповіді не редагується.',
  threshold:
    'Виділений фрагмент тексту → кандидат у поріг (значущий вибір, ризик, ціна, незворотність). AI лише пропонує — автор підтверджує чи редагує перед збереженням; score()/is_real_threshold() рахує завжди сервер. Справжній збережений поріг дописує доказ у навичку WDI «Робота з персонажем». Схема відповіді не редагується.',
  instructionElaboration:
    'Напрям «Інструкція» експрес-майстра, крок завершення: готовий документ (усі кроки разом) → поглиблене пояснення виконання КОЖНОГО кроку. Результат лягає в текст нової книги як [AI-DRAFT] — автор бачить, що це доповнення ШІ, і сам вирішує прийняти чи відхилити. Схема відповіді не редагується.',
};
...
const CORE_FIELD_DEFS: Record<string, { key: string; label: string; textarea?: boolean; placeholder?: string }> = {
  ...
  '{{genre}}': { key: 'genre', label: 'Жанр книги', placeholder: 'фантастика' },
  '{{audience}}': { key: 'audience', label: 'Аудиторія', placeholder: 'дорослі читачі' },
  '{{chapterCount}}': { key: 'chapterCount', label: 'Скільки розділів', placeholder: '12' },
  '{{wordCount}}': { key: 'wordCount', label: 'Приблизний обсяг, слів', placeholder: '48000' },
  '{{sample}}': { key: 'sample', label: 'Зразок тексту (початок книги)', textarea: true },
  '{{task}}': { key: 'task', label: 'Тип задачі', placeholder: 'ціна / тренд / лістинг / магазин' },
  '{{question}}': { key: 'question', label: 'Питання й дані', textarea: true },
  // «Емоційна майстерність письменника» + «Поріг» — нові поля, яких немає в жодному з попередніх модулів.
  '{ПЕРСОНАЖ}': { key: 'characterName', label: 'Персонаж' },
  '{ПРОФІЛЬ_ПЕРСОНАЖА}': { key: 'characterProfile', label: 'Профіль персонажа (контекст)', textarea: true },
  '{ОПИС_СЦЕНИ}': { key: 'sceneSummary', label: 'Опис сцени' },
  '{ПОТОЧНИЙ_ПОРІГ}': { key: 'currentThreshold', label: 'Поточний поріг персонажа (контекст)', textarea: true },
  '{КОНТЕКСТ_ДО}': { key: 'previousParagraph', label: 'Попередній абзац (контекст)', textarea: true },
};
