...
/**
 * Знаходить у готовому документі ВЕРХНЬОБЛОКОВИЙ діапазон, у якому є `needle`.
 *
 * Навіщо це, якщо є `markerOffsetToDocPos` вище. Той рахує довжини маркерів і
 * зіставляє їх зі зміщенням у СИРОМУ рядку книги; якщо збережений текст
 * відрізняється від того, у що його перетворює редактор (нормалізація
 * маркерів, абзаци, що злились), арифметика розходиться — і підсвічування
 * МОВЧКИ не зʼявляється. Саме це знайшов живий прогін #224: у реальному
 * розділі книги відображення зміщення повертало null, автор бачив текст без
 * жодного виділення й без прокрутки.
 *
 * Тут навпаки: питаємо САМ документ, у якому блоці лежить щойно вставлений
 * текст. Для наших потреб цього досить: і передача з медіатеки, і міст
 * «чат → книга» дописують текст у КІНЕЦЬ секції окремими абзацами, тож
 * потрібний блок — перший, у якому є пошуковий рядок.
 *
 * `null` — не знайдено (напр. редактор ще не завантажив контент): виклик сам
 * вирішує, чи пробувати ще раз, чи відкотитись на відображення зміщень.
 */
export function findBlockRangeWithText(doc: PMNode, needle: string): { from: number; to: number } | null {
  ...
  doc.forEach((node, offset) => {
    ...
    if (!text || !text.includes(trimmed)) return;
    // +1 / −1 — всередину блоку (позиції ProseMirror на один крок ширші за
    // текст: відкриття й закриття вузла), тобто виділяємо сам вміст блоку.
    found = { from: offset + 1, to: offset + node.nodeSize - 1 };
  });
  return found;
}
