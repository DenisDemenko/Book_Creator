 ...
 * «Літературна критика» = 118; зв'язків 21 + 16 = 37 (+2 часові з ТЗ = 39). Усі п'ять чисел
...
console.log('\n── Склад реєстру: числа мусять збігтися з документом ──');
{
  ...
  eq('зв’язків 39 (37 з документа + 2 з ТЗ)', stats.relations, 39);
...
console.log('\n── Цілісність записів ──');
{
  ...
  eq('зв’язків із ТЗ «11 сторінок» — follows і overlaps',
    CORE_ENTITY_RELATIONS.filter((r) => r.registry === 'spec').map((r) => r.key).join(','), 'follows,overlaps');
