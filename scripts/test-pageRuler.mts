...
function main() {
  ...
  console.log('\nЛінійка живе ВСЕРЕДИНІ колонки — один контейнер, один масштаб (фаза 4):');
  {
    ...
    // `sticky top-0` саме як підрядок тепер зустрічається двічі в розмітці
    // PageColumn: у рядку лінійки (`z-20`) і в НЕЗАЛЕЖНОМУ підписі формату
    // аркуша (`z-30`, запис #193) — той рендериться завжди, коли є
    // `pageGeometry`, незалежно від того, показана лінійка чи ні. Тому
    // перевіряємо саме рядок лінійки (клас `z-20`), а не підрядок, спільний
    // тепер із іншим, свідомо завжди видимим елементом.
    t('без лінійки рядок лінійки не рендериться зовсім', !renderColumn(null).includes('sticky top-0 z-20'));
    t('підпис формату аркуша рендериться незалежно від лінійки', renderColumn(null).includes('sticky top-0 z-30'));
  }
