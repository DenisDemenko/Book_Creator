...
export class MarketplaceBridgeError extends Error {
  constructor(
    ...
    readonly kind:
      ...
      | 'unreachable'
      | 'bad_url'
      /** Немає зображення для картки — публікацію зупинено (див. нижче). */
      | 'cover_required',
    readonly status = 500,
...
/**
 * Картка без зображення у вітрину не йде.
 *
 * Знахідка живого прогону вітрини (запис #168): серед опублікованого зі
 * Студії знайшлись картки з ПОРОЖНІМ `coverUrl` — курс і тестова книга. У
 * каталозі вони показуються порожнім прямокутником, і саме це власник назвав
 * «книга на вітрині без фотографій».
 *
 * Доти правило «без обкладинки не публікуємо» існувало ЛИШЕ на шляху книжки з
 * браузера (`/api/admin/pdf/publish`, там перевірка `cover_required`
 * з'явилась раніше). Курси й адмінські публікації йшли з `coverUrl`, якого
 * могло не бути взагалі, — і про це ніхто не дізнавався, бо відмови не було.
 *
 * Тепер правило одне на всі шляхи, і воно навмисно ЖОРСТКЕ: краще відмовити з
 * причиною, ніж поставити в каталог порожню картку, яку автор побачить через
 * тиждень і вже не згадає, звідки вона.
 */
export function assertStorefrontCover(coverUrl: unknown, what = 'Картка'): void {
  const value = String(coverUrl ?? '').trim();
  if (value) return;
  throw new MarketplaceBridgeError(
    `${what} без обкладинки у вітрину не піде: у каталозі вона показується порожнім прямокутником, ` +
      'а порожня картка не продає, а знецінює. Додайте обкладинку (посилання на зображення) і повторіть публікацію.',
    'cover_required',
    400
  );
}
 ...
 * у `/bridge/books`.
 *
 * Приймач `/bridge/courses` на боці Fusion Lab підтверджений і
 * реалізований (перевірено в репозиторії вітрини: `bridge.controller.ts`
 * → `bridge.service.ts#publishCourse`, е2е `integrations.e2e-spec.ts`)
 * — раніше ця функція будувала лише свою половину контракту наосліп,
 * тепер обидві сторони узгоджені.
 *
 * Обсяг полів (аудит log.md #137): картка-вітрина — усе, що покупець
 * читає ДО оплати, щоб вирішити, чи купувати курс: валюта, категорія,
 * аудиторія, результати навчання, навички (назва, рівень, навіщо) і
 * скелет програми (назви модулів/уроків, тривалість). log.md #138
 * додав до цього посилання на відео уроку/модуля (`videoUrl`/
 * `introVideoUrl`) — Nova не перевіряє й не розрізняє провайдера
 * (YouTube чи Vimeo — вибір автора курсу, просто рядок URL), а вітрина
 * ховає його з публічної відповіді каталогу й показує лише покупцю
 * (`stripPaidCourseContent` + `Entitlement`, те саме, що вже захищає
 * платні файли). Досі свідомо НЕ передається решта платного матеріалу —
 * опис/мета уроку, тексти завдань і критерії прийняття: цей контент,
 * як і файли книг, лишається за окремим кроком (за взірцем
 * `attachBookFileToMarketplace`), не за карткою.
 */
...
export interface PublishCourseModuleLesson {
  ...
  durationMin?: number;
  /**
   * Посилання на відео уроку — YouTube чи Vimeo, на вибір автора курсу;
   * Nova не парсить і не валідує провайдера, просто передає рядок далі.
   * Платний контент: вітрина ховає його з публічного каталогу й показує
   * лише покупцю з активним `Entitlement` (`stripPaidCourseContent`
   * у Fusion Lab).
   */
  videoUrl?: string;
}
...
export interface PublishCourseModule {
  title: string;
  summary?: string;
  /** Оглядове відео модуля — той самий провайдер-агностичний рядок, що й `PublishCourseModuleLesson.videoUrl`. */
