...
export interface PdfLayoutSpec {
  ...
  /**
   * Дзеркальні поля: `margins.left` стає ВНУТРІШНІМ (корінець), `right` —
   * зовнішнім, і на парних сторінках вони міняються місцями.
   *
   * Для екранного PDF це косметика, для друкованої книги — ні: без корінця
   * текст біля згину або нечитабельний, або обрізається при переплетенні.
   * KDP прямо відхиляє макет, де внутрішнє поле замале для обсягу.
   *
   * Сума полів при цьому стала, тож ширина набору не змінюється — зсувається
   * лише його положення на сторінці.
   */
...
export interface PdfBookInput {
   ...
   * Ілюстрації.
   *
   * ДО #169 ТУТ БУЛО НАПИСАНО «книга не зберігає точного місця вставки» — і
   * це було неправдою. Місце зберігає САМ РУКОПИС: у тексті стоїть маркер
   * `[IMG: id "підпис" wrap=…]`, рівно там, куди автор поклав картинку
   * (конвенція з `utils/imageMarkers.ts`, у книгах із .docx — з #159). Поки
   * рендерер його не бачив, він друкував маркер голим рядком, а всі
   * зображення глави скидав купою в її кінці: у книзі на 373 сторінки 39
   * сторінок мали замість картинок текст `[IMG: docx-img-N …]`.
   *
   * Тепер картинка малюється НА МІСЦІ маркера, а поля нижче лишаються
   * сумісним шляхом для книг, написаних до появи маркерів.
   */
  illustrations?: Array<{ id?: string; chapterId?: string; url: string; caption?: string }>;
  /**
   * Картинка за id маркера: ілюстрація, портрет героя (`char-…`) або
   * обкладинка (`cover-front`). Складається в `pdfFromBook.ts` через
   * `buildMarkerImageMap` — рендерер не бачить ні героїв, ні обкладинки,
